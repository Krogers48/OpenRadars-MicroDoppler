...
if __name__ == "__main__":
    ...
    theta_ghost_smooth = medfilt(np.where(np.isfinite(theta_ghost_curve), theta_ghost_curve, 0.0), kernel_size=5)
    theta_ghost_smooth[~np.isfinite(theta_ghost_curve)] = np.nan

    a_use = np.nanmedian([a_22, a_23, a_24])
    theta_theory = None
    ...
    if theta_theory is not None and np.any(np.isfinite(theta_theory)):
        plt.plot(theta_theory, ranges_m, 'r:', label='theoretical angle')
    if np.any(np.isfinite(theta_ghost_smooth)):
        plt.plot(theta_ghost_smooth, ranges_m, 'y--', label='noise reduced')
